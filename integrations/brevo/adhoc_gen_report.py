...
from .modify_report_metadata import modify_report_metadata
import time
import os
...
def plot_stats(stats, title):
    ...
    if not stats['campaignStats']:
        ...
        plt.savefig(f'{app_files}/users/{user_id}/reports/{title}.png', format='png')
        plt.close()
        img.seek(0)
...
def write_contact_lists(campaign_name, recipients):
    with open(f"{app_files}/users/{user_id}/email_lists/{campaign_name}.csv", "w") as file:
...
def initiate(user_id):
    modify_report_metadata()
    campaigns = recent_campaign_reports()
    #Archive report before each run
    current_utc_date = time.strftime("%Y-%m-%d", time.gmtime())
    shutil.copy2(f'{app_files}/users/{user_id}/campaign_report.html', f'{app_files}/users/{user_id}/reports/{current_utc_date}-E3-Report.html')
    ...
    env = Environment(loader=FileSystemLoader(f'{app_files}'))
    template = env.get_template(f'report_template.html')
    notify_template = env.get_template(f'notify_template.html')

    # Render the template with data
    html_content = template.render(campaigns=campaigns, fqdn=fqdn, port=port)
    notify_html_content = notify_template.render(campaigns=campaigns, fqdn=fqdn, port=port)

    # Write the HTML content to a file
    with open(f'{app_files}/users/{user_id}/{current_utc_date}-E3-Report.html', 'w') as file:
        file.write(html_content)

    # Write report formatted for email
    with open(f'{app_files}/users/{user_id}/templates/notify.html', 'w') as file:
        file.write(notify_html_content)
